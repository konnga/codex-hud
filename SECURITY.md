...
Optional external usage snapshots must use absolute `.json` paths. Written snapshots use private file permissions on POSIX systems. Relay balance queries are disabled by default and require an explicit setup choice or configuration entry. Authentication detection reports only the method and an optional non-secret account label; API keys and access tokens are never rendered.
