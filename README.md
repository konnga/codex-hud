...
While quota is visible, ChatGPT sessions with local `auth.json` credentials also refresh the account's `codex` quota bucket through the official [app-server account API](https://learn.chatgpt.com/docs/app-server#6-rate-limits-chatgpt), even when no rollout events arrive. HUDs sharing a Codex home and account share one read per minute. The background reader has a ten-second timeout and does not start a conversation; Codex handles authentication. Account snapshots use private files partitioned by a hash of the workspace and user, without storing credentials. API-key and relay sessions do not use this reader; keyring-only or unavailable credentials fall back to observed telemetry.

Newer observations take precedence over older caches, including when a quota reset lowers usage. A failed refresh preserves the last successful snapshot and its observation time. Failed reads and observations older than 90 seconds display `cached, updated …` beside the percentage. The time is when HUD obtained the observation, not the quota reset time. `doctor --json` reports the selected source/time, whether account polling is available, and the last attempt/failure state.
