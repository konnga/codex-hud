...
async function runRenderCli(args = process.argv.slice(2)) {
	...
	const renderFrame = async () => {
		...
		const queriedUsage = loaded.config.display.showAuth ? options.once ? await readConfiguredExternalUsage(loaded.config.display.externalUsageQueries, endpoint?.url ?? null, process.env, now.getTime(), rollout.session) : readCachedConfiguredExternalUsage(loaded.config.display.externalUsageQueries, endpoint?.url ?? null, process.env, () => void render(), now.getTime(), rollout.session) : null;
